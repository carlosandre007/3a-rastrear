    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {deferredPrompt && (
            <button onClick={handleInstall} className={`ml-4 font-bold text-sm ${isScrolled ? 'text-secondary' : 'text-white'} hover:text-primary`}>Instalar App</button>
          )}
          ...
          <button
            ...
            className="bg-primary hover:bg-yellow-500 text-secondary px-6 py-2.5 rounded-xl font-black text-sm shadow-lg transition-all active:scale-95 text-center inline-block"
          >
            EM BREVE
          </button>
        </div>
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-2xl p-6 flex flex-col gap-4 border-t animate-fade-in-down">
          ...
          <button
            onClick={() => {
              setIsOpen(false);
              alert("A Área do Cliente está em atualização e estará disponível em breve.");
            }}
            className="w-full bg-secondary text-white py-4 rounded-xl font-bold mt-2 text-center block"
          >
            Em breve
          </button>
          <a
